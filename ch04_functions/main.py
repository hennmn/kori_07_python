...
그러면 여태까지 input() 이라는 파이썬 내장함수를 사용하고 있었다고 볼 수 있습니다. 파이썬 내장 함수는 이미 정의가 되어있고, 개발자들은 함수 호출만 잘 하면 되겠네요.
...
2. 메서드(methods) : 특정 객체가 가지고 있는 함수를 의미. 특정 자료형에 포함되어 있는 함수. 사실 함수와 메서드는 동일한 개념이지만, 호출 방식에 있어서의 차이가 있습니다.
...
'''
700원 짜리 음료수를 뽑을 수 있는 자판기 프로그램을 구현하시오. 
돈을 넣으면 몇 잔의 음료수를 뽑을 수 있는 지,
그리고 잔돈은 얼마인지 모든 경우의 수를 출력하도록 합니다.

함수 정의
- 반환값 : 없음(call1 ~ call4 중 어떤 유형일지 고민할 필요가 있습니다.)
- 함수 이름 : vendeng_machine()
- 매개변수 : 정수 meney
...
# my_money를 기준으로 음료수를 살 수 있는 경우의 수를 고려했을 때 0~4개 까지 반복문이 5번 돌아갑니다.
...
def vending_machin(money):
    ...
    for i in range(money // drink_price + 1):
        print(f'음료수 개수 = {i}개, 잔돈 = {money - (drink_price * i)}')

vending_machin(3000)

def vending_machin2(money):
    drink_price = 700
    i = 0
    while True:
...
함수 호출 : multiply(dan)   // argument가 dan
...
그럼 현재 multiply를 call2() 유형으로 정의했다고 볼 수 있습니다.
