...
# 함수 정의 영역
def is_resource_enough(order_ingredient): #만약에 특정 함수/메서드의 결과값이 boolean이라면 대개 다음 조건문/반복문의 조건식으로 쓰이는 경우가 많습니다. 함수형 프로그래밍 개념을 떠올리시면 됩니다.
    """
...
# todo - 3 : 사용자가 프롬프트에 "report"를 입력하면 현재 자원값을 보여주는 보고서를 생성합니다.
    elif choice == 'report':
...
# todo - 4 : choice == 에스프레소 / 라떼 / 카푸치노 중 하나일 때 작성하는 부분
        ...
        # '에스프레소' or '라떼' or '카푸치노' 이게 자바스러운 방식
        # todo - 6 : 사용자가 음료를 선택하면 프로그램은 해당 음료를 만들기에 충분한 자원이 있는 지 확인해야 합니다.
        # 자판기에서 choice에 정확한 음료 이름을 입력했을 때의 처리과정 (메뉴얼 참조)
        # 1. 자원이 충분한 지 확인(T / F)
        # 2. T라면 돈을 입력 받을 수 있도록 -> 동전 입력 받아서 합 연산한 후에 음료의 가격 이상인지 확인하여 T / F 반환
        # 3. T라면 음료를 반환해야 하는데, resources에 있는 수량을 감소 시키고, profit에는 음료가격만큼 증가시킬 필요 있음. 그리고 음료 가격반큼 빼고 반환도 해줘야하 함.
    ...
    # todo = 5 : choice가 이상의 조건을 충족하지 않을 때 '잘못입력하셨습니다' 다시 입력하세요'를 출력하는 부분
    else :
