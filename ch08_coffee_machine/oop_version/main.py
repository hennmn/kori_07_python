...
# Python 객체를 만드는 방식이죠. 이전과 차이가 있다면 외부 모듈(파일)에서 가지고 온 클래스의 객체를 생성했다는 점입니다. Scanner scanner = new Scanner(System.in);과 같습니다.

# 그럼 이제 뭐할거냐면 coffee_maker / money_machine / menu 모듈은 하나도 안 건드리고 main에만 코드를 작성해서 pop version과 동일한 기능을 하는 coffee_machine을 만들겁니다.
menu = Menu()
...
while is_on:
    ...
    if choice == 'off':
        ...
        # if choice not in ['espresso', 'cappuccino', 'latte']:  얘는 메뉴 추가될 때마다 수정해야 해서 좀,,
        #     continue
        ...
        # 음료 이름을 입력 받은 시점부터의 프로세스를 떠올려서 코드를 작셩하셔야만 합니다. 이때 고려해야 할 것은 절차지향으로 코딩했을 때의 과정과, 현재 참조해야 하는 파이썬 모듈들의 매개변수 차이와 메서드 작동 원리의 차이를 감안해서 코드를 쓸 필요가 있다는 점입니다.
