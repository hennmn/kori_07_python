...
type() : 소괄호 내에 있는 데이터(argument)가 어떤 자료형인지 알려주는 명령어
    - JS 에서는 typeof가 함수가 아니라 연산자였습니다.
'''

print('python 수업을 시작했습니다. 파이팅')
print('''
이렇게 다중으로 작성하고 싶을 때는 \'\'\'\'\'\' 으로 작성하는 벙법도 있습니다.
Java에서는 줄 넘어갈 때 마다 +로 연결해줘야 했는데 그런 점은 편합니다.''')

'''
이상에서 알 수 있는 점은 print()가 아니라 System.out.println()처럼 자동 개행이 된다는 점입니다.
...
'''
python은 좀 까탈스러워서 Java나 JS 할 때 처럼 맨 처음이 str이면 뒤의 int/float을 알아서 바꿔주는 짓을 안합니다.
...
console.log(`안녕하세요, 제 이름은 ${name}이고, 나이는 ${age}살 입니다.`);의 python 판이라고 생각하시면 됩니다.
