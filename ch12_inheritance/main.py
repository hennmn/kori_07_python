...
'''
1. 서브클래스의 __init__()
    서브클래스는 슈퍼 클래스가 없으면 존재할 수 없습니다. 그래서 서브 클래스의 생성자를 구현할 때는 '반드시 슈퍼클래스의 생성자를 먼저 호출'하는 코드를 작성해야만 합니다. 
    
    super().에서 super -> 슈퍼 클래스를 의미. 즉 이상의 코드에서 Student의 생성자를 호출하려면 super().__inint__(name)에 의해서 슈퍼 클래스인 Person의 생성자가 먼저 호출되면 '슈퍼 클래스의 객체가 생성' 됩니다. 이후에 슈퍼 클래스에서 생성된 인스턴스 변수인 name이 서브 클래스로 전달되고, 이후에 서브 클래스에서 school 인스턴스 변수를 선언 및 초기화하여 저장하면서 서브 클래스의 인스턴스가 생성됩니다.
...
2. 서브 클래스의 인스턴스 자료형
    ...
    Java를 기준으로 instanceof 연산자 역할을 하는 함수가 python에도 있는데 -> isintanceof()  : 다 소문자 입니다.
...
2. 서브 클래스 Hybrid 는 다음과 같은 특징을 지니고 있습니다.
    1) 최대 배터리 충전량은 30
    2) 배터리를 충전하는 charge() 메서드가 존재합니다. 최대 충전량은 초과할 수 없고, 0보다 작은 값으로 충전할 수 없습니다.
    3) 현재 주유량과 충전량을 모두 확인할 수 있는 hybrid_info() 메서드가 있습니다.
    
3. 다음과 같은 방식으로 전체 동장을 확인할 수 있습니다.
...
#             return      # 메서드 종료
#         self.oil += oil     # 이상의 조건문이 실행되지 않으면 속성에다가 oil 만큼 더해줍니다.
...
#
# class Hybrid(Car):
#     max_charge = 30     # 부모 클래스 변수 접근 가능하니까 추가 클래스 변수를 정의 및 초기화
#
#     def __init__(self, oil, amount):
#         super().__init__(oil)
#         self.amount = amount
#         print('하이브라이드 차량이 생산되었습니다.')
#
#     # 부모 클래스의 메서드를 오버라이드
...
#
#     # 고유 메서드를 정의했는데 부모 메서드 가지고 오고 추가함. 저 같으면 그냥 car_info() override해서 재정의 했을 것 같습니다.
#     def hybrid_info(self):
#         # 오버라이드 하는 건 아니고 부모 메서드를 그대로 호출
#         super().car_info()
...
2. Shape 클래스를 상속 받는 서브 클래스 Circle을 정의하시오.
    ...
    - 생성자에서 radius도 추가할 것
    - area() 메서드를 정의히야 원의 넓이를 계산하고 return 할 것.  -> call3()
...
class Circle(Shape):
    def __init__(self, name, radius):
        super().__init__(name)
        self.radius = radius
        print(f'반지름이 {radius}인 원이 생성되었습니다.')
...
class Rectangle(Shape):
    ...
    def draw(self):
        # super().draw()   override 했지만 꼭 사용하지 않아도 됨
