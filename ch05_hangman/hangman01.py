...
'''
# numbers = [1,2,3,4,5]
# chosen_number = random.choice(numbers)  # choice() 라는 메서드의 argumnet로 list를 넣었습니다.
# print(chosen_number)
...
# 그리고 대문자로 시작하는 경우를 방지하기 위해 input() 함수 뒤에 .lower()를 적용하시오.
# guess = input('알파벳 하나를 입력하시오 >>> ').lower()
...
'''
예를 들어 chosen_word가 apple이고, guess에 a 를 입력했다면
정답
이라고 출력될 것.
b라고 입력했다면
오답이라고 출력될 것
...
# #2 일반 for
# len() 함수 : 반복 가능 객체의 길이를 int로 return 하는 함수
# 이거 내가 한 거랑 똑같음