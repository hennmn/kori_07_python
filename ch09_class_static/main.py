...
'''
객체명.클래스변수를 통해서 클래스 변수에 접근이 가능하긴 하지만, 클래스 내부의 코드를 들여다보기 전 까지는 country라는 변수가 인스턴스 변수인지 클래스 변수인지 알 방법이 없다는 문제가 있습니다.

이상의 이유로 클래스 변수를 확인하고자 할 때는 객체명.클래스변수명 보다는
클래스명.클래스변수명을을 통해서 참조하는 것이 권장됩니다.
print(Korean.country) 이렇게 
...
3. 정적 메서드(static method)
    ...
    특징 : 
        ...
        2) 생성된 인스턴스가 없어도 호출 가능 -> 클래스 메서드와의 공톰점
        
        3) @staticmethod 데코레이터를 표기하고 작성 -> 클래스 메서드와의 차이점 #1
        4) 반드시 작성해야 할 매개변수(self / cls) 가 없음 -> 클래스 메서드와의 차이점 # 2
        
이상을 토대로, 정적 메서드는 self / cls 를 둘 다 사용하지 않기 때문에 인스턴스 / 클래스변수를 모드 사용하지 않는 메서드를 정의하는 겨웅에 적합합니다. 정적 메서드 자체는 클래스에 소속돼있지만 인스턴스에는 영향을 주지도 않고 받지도 않습니다.
...
class Bag:
    ...
    def __init__(self):  # 생성자 호출 및 인스턴스 변수들 정의할 용도. 그럼 생성자도 인스턴스 변수라고 할 수 있겠네요.(self가 있으니까
        Bag.count += 1 # 생성자가 호출될 때마다(=객체가 생성될 때마다) 클래스 변수인 count가 1씩 증가함. cls.count가 아니라 클래스명.count 라는 것에 주목해야 합니다.
...
# bag3 = Bag()
# print(f'현재 가방 재고 : {Bag.remain_bag()}')
# bag1.sell()   # 실제가 bag1 객체가 사라진 건 아닌 거 아시죠?
# print(f'현재 가방 재고 : {Bag.remain_bag()}')
# bag2.sell()
# print(f'현재 가방 재고 : {Bag.remain_bag()}')
...
2. man과 woman 인스턴스가 생성되면 다음고 같은 메시지를 출력할 수 있도록 작성하시오.
김일이(가) 태어났습니다.
김이이(가) 태어났습니다.
...
5. 다음과 같은 명령어가 실행되여 man 인스턴스가 삭제되면 해당 메시지가 출력되게끔 작성하시오.
RIP 김일
...
'''
다음 지시사항을 읽고 가게의 매출을 구하는 Shop 클래스를 작성하시오.
